...
import {
  View, Text, TouchableOpacity, StatusBar, StyleSheet, ScrollView, Image, Modal, TextInput, Alert, Dimensions
} from 'react-native';
...
export default function PresupuestosMensuales({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => { loadData(); }, [currentMonthIndex, currentYear])
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updateBudget(editingId, category, parseFloat(limit));
        Alert.alert("Éxito", "Presupuesto actualizado");
      } else {
        await addBudget(userId, category, parseFloat(limit), mesNombre, anioStr);
  ...
  const handleDelete = (id) => {
    Alert.alert("Eliminar", "¿Borrar este presupuesto?", [
      ...
      { text: "Eliminar", style: "destructive", onPress: async () => { await deleteBudget(id); loadData(); } }
    ]);
  };
  ...
  const openEdit = (item) => {
    ...
    setLimit(item.monto_limite.toString());
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <Text style={styles.screenTitle}>Mis Presupuestos</Text>
        ...
        {budgets.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubText}>Define límites para controlar tus gastos.</Text>
          </View>
        ) : (
          budgets.map((b) => {
      ...
      <TouchableOpacity 
        style={styles.fab} 
        onPress={() => { setEditingId(null); setCategory(''); setLimit(''); setModalVisible(true); }}
      >
...
const styles = StyleSheet.create({
  ...
  fab: { position: 'absolute', bottom: 20, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: '#0D7A43', justifyContent: 'center', alignItems: 'center', elevation: 8, shadowColor: '#0D7A43', shadowOpacity: 0.4, shadowOffset: {width: 0, height: 4} },
